...
**Opción A - Usar el backup completo (RECOMENDADO):**

Este archivo contiene toda la estructura y datos actuales del proyecto.

```powershell
docker exec -i sistemafoodix-db-1 mysql -u root -p'root' db_foodix < backup_db_foodix.sql
```

**Opción B - Cargar solo datos iniciales básicos:**

Si prefieres empezar desde cero con datos mínimos.

```bash
docker exec -i sistemafoodix-db-1 mysql -u root -p'root' db_foodix < datos_iniciales.sql
```

**Opción C - Desde un cliente MySQL (DBeaver, MySQL Workbench, etc.):**
...
2. Ejecutar el script `backup_db_foodix.sql` (o `datos_iniciales.sql`)
